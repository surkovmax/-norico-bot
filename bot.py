...
ALLOWED_THREAD_ID = None  # будет установлен при первом /start в группе
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global ALLOWED_THREAD_ID
    if update.message.message_thread_id:
        ALLOWED_THREAD_ID = update.message.message_thread_id
        print(f"Закреплён топик THREAD_ID={ALLOWED_THREAD_ID}")
    keyboard = [
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"THREAD_ID={thread_id} ALLOWED={ALLOWED_THREAD_ID}")
    if ALLOWED_THREAD_ID and thread_id != ALLOWED_THREAD_ID:
        return  # игнорируем сообщения из других топиков
